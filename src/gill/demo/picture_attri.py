...
def plot_overlap_counts(top_k_indices, save_dir):
    ...
    for k, (unique_indices_2d, counts) in top_k_indices.items():
        ...
        plt.bar(range(1, 11), count_values.cpu().float().numpy(), label=f'Top {k}')
        ...
        plt.close()
def pic_value_mean(paths,save_dir):
    # 加载并处理张量
...
def pic_value_img0(paths,save_dir):
    # 加载并处理张量
    tensors = [torch.unsqueeze(torch.load(file_path), 0) for file_path in file_paths]# （8，32，16384）
    ...
    for k in [50, 100, 500]:
        ...
        # 单批次·
        for tensor in stack_tensor:
            values, indices = torch.topk(tensor.view(-1), k) # （32，16384）->(32*16384)
            top_k_indices[k].append(indices)
...
def find_proj(model, layers_list, token_id):
    ...
    expanded_project = torch.cat([project_new.unsqueeze(0)] * layers_list.shape[0], dim=0)
...
def pic_value_Wout(path, save_dir, path_wout):
    ...
    print(wout.device)
    for i in range(10):
        expanded_project_i = torch.sum(stack_tensor[i].unsqueeze(-1) * wout, dim=-1).unsqueeze(0)  # (1,32,16384)
    ...
    # 打印前50个最大值的索引及其重合次数
    plot_overlap_counts(top_k_indices, save_dir)
def pic_value_Wout0(path, save_dir, path_wout):
    tensors = [torch.unsqueeze(torch.load(file_path), 0) for file_path in file_paths]
    stack_tensor = torch.cat(tensors, dim=0).permute(1, 0, 2, 3) # (8,10,32,16384)
    stack_tensor = stack_tensor[0]  # (10,32,16384)
...
def meaning(path, Wreadout_Wout_path,wout_path): # (32,16384,16384) 表示的是分数第二维：表示16384个词的得分
    W = torch.load(Wreadout_Wout_path) # (32,16384,16384) 第二维表示神经元，表示的是分数第三维：表示16384个词的得分
    tensors = [torch.unsqueeze(torch.load(file_path), 0) for file_path in file_paths]
    stack_tensor = torch.cat(tensors, dim=0).permute(1, 0, 2, 3)
    stack_tensor = stack_tensor[0] # (10,32,16384)
    ...
    tokenizer = AutoTokenizer.from_pretrained("/data/ruip/eva02/gill_done/transformer_cache/opt") # 分词器
    values, indices = torch.topk(stack_tensor.view(-1), 5) #（32，16384）中前5个最大值
    row_indices = indices // 16384
    col_indices = indices % 16384
    top_k_indices = torch.stack((row_indices, col_indices),dim=0) # 神经元索引（层数+该层索引）
    for i in range(top_k_indices.shape[1]): #5:0-4
        values,indices = torch.topk(W[top_k_indices[0][i]][top_k_indices[1][i]],5)
...
def calculate_scores(Wout, weights, acts): # 如果输入的acts的结构是(10,8,32,16384)
    ...
    score = torch.sum(norm_acts * Wout * weights, dim=-1) # (10,8,32,16384,1)*(1,1,32,16384,4096):(10,8,32,16384,4096)*(10,8,1,1,4096)=(10,8,32,16384)
