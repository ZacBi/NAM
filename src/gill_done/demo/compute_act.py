...
def deal_img(path):
    ...
    return files_list
def find_eva_act(path, input_text, ret_scale_factor, num_words, temperature):
    to_return = []
    to_return_1 = []
    model_inputs = []
    model_outputs = []
    ...
    for index, model_input in enumerate(model_input_s): 
        with torch.no_grad():
            ...
            layers_act[index] = find_token("dog", input_text, act, model_outputs)
    # 对处理结果求平均
    ...
    for output_i, p in enumerate(model_outputs):
        if type(p) == str:
            ...
            if p['decision'] is not None and p['decision'][0] == 'gen':
                ...
                image.save('/data/ruip/image/output/{}.jpg'.format(index))
            else:
...
handle_act = [model.model.lm.model.decoder.layers[n].activation_fn.register_forward_hook(forward_hook(n)) for n in
              range(32)]    
    # 返回更新后的状态和对话记录。l
path_1 = '/data/ruip/eva02/test_dog_data1'
...
for i in range(1):
    print("生成第{}个".format(1))
    model_inputs.append(files_list[0])
    with torch.no_grad():
...
result = [(tensor / 3) for tensor in result]
